...
编写conf很累，然后官方镜像没有很全面，很多无法使用。如果你k8s用ingress，则无法使用http3。  
于是一个尽量精简，安装使用方便，通过源码编译构建，权限锁死的nuoyis's build lnmp出现了  
仅需启动容器，则有模板可以粘贴，甚至头部只需include即可编写完毕，无需配置一堆头部内容  
也保留了原版写法，满足个人的个性化。  
搭配nuoyis-linux-toolbox，仅需敲一行代码即可部署完毕  
