...
RUN case "$TARGETARCH$TARGETVARIANT" in \
      ...
      armv7)  export CC=arm-linux-gnueabihf-gcc CXX=arm-linux-gnueabihf-g++ CFLAGS="-march=armv7-a -mfloat-abi=hard -O2" CXXFLAGS="-march=armv7-a -mfloat-abi=hard -O2" ;; \
      armv6)  export CC=arm-linux-gnueabi-gcc CXX=arm-linux-gnueabi-g++ CFLAGS="-march=armv6 -mfloat-abi=soft -O2" CXXFLAGS="-march=armv6 -mfloat-abi=soft -O2" ;; \
...
RUN CONFIGURE_OPTS="--prefix=/nuoyis-web/openssl-1.1.1 --openssldir=/nuoyis-web/openssl-1.1.1 no-shared no-dso no-tests"; \
    if [ "$TARGETARCH" = "arm" ] && [ "$TARGETVARIANT" = "v7" ]; then \
        ./Configure linux-armv4 $CONFIGURE_OPTS; \
